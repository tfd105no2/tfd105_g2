...
// 獲取類型
$ordervalue = $_POST['orderTypeValue'];

// sql
$sql = "SELECT * FROM order1 where order_type = '$ordervalue'";
...
// sql
$sql = "SELECT * FROM team2_db.order1 a 
        join member b on a.member_id = b.id 
        where order_type = '$ordervalue' limit $start,$per";
// 執行sql語句
$statement = $pdo->query($sql);
// 將查詢到的資料抓出放到$data
if ($statement) {
